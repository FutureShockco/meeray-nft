...
onMounted(async () => {
  try {
    ...
    const [nftData, collectionInfo] = await Promise.all([
      api.getNftInstance(`${collection}-${nftId}`), // Using getNftInstance with proper ID format
  ...
  try {
    ...
    result.onStatusChange((status) => {
      if (status.status === 'COMPLETED') {
        ...
        transferTo.value = ''
        router.push(`/profile/${transferTo.value}`)
      }
    })
  } catch (err) {
...
const listNFT = async () => {
  ...
  try {
    ...
    const listingData: any = {
      ...
      paymentTokenSymbol: listingForm.value.paymentToken,
        ...
        <div class="space-y-6">
          ...
          <div class="nft-panel">
            ...
            <div v-if="nft.properties?.length" class="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div 
                v-for="trait in nft.properties" 
                :key="trait.trait_type"
                class="bg-gray-800 rounded-lg p-3 text-center"
              >
                <div class="text-xs text-gray-400 uppercase">{{ trait.trait_type }}</div>
                <div class="text-sm font-semibold text-white">{{ trait.value }}</div>
              </div>
            </div>
        ...
        <div class="space-y-6">
          ...
          <div class="nft-panel">
            <div v-if="nft.isListed" class="mb-6">
              ...
              <div class="text-3xl font-bold text-white">
                {{ nft.price }} {{ nft.paymentToken || 'STEEM' }}
              </div>
            </div>
            ...
            <div class="space-y-3">
              ...
              <button 
                v-if="canBuy" 
                @click="buyNFT"
                class="nft-btn w-full py-4 text-lg"
              >
              ...
              <button 
                ...
                class="nft-btn w-full py-4 text-lg bg-purple-600"
              >
                Place Bid
              </button>
              ...
              <div v-if="isOwner" class="space-y-3">
                ...
                <button 
                  v-if="nft.isListed"
                  @click="delistNFT"
                  class="nft-btn w-full py-3 bg-red-600"
                >
                  Remove from Sale
                </button>
          ...
          <div class="nft-panel">
            <div class="flex border-b border-gray-700 mb-4">
              <button 
                v-for="tab in ['details', 'history', 'offers']" 
                :key="tab"
                @click="activeTab = tab"
                :class="[
          ...
          <div class="space-y-6">
            ...
            <div v-if="listingForm.listingType === 'AUCTION' || listingForm.listingType === 'RESERVE_AUCTION'">
              ...
              <div v-if="listingForm.listingType === 'RESERVE_AUCTION'">
                ...
                <input 
                  v-model="listingForm.reservePrice" 
                  type="number" 
                  step="0.001"
              ...
              <div>
                ...
                <input 
                  v-model="listingForm.auctionEndTime" 
                  type="datetime-local"
                  class="w-full steem-auth-input"
                >
              </div>
              ...
              <div>
                ...
                <input 
                  v-model="listingForm.minimumBidIncrement" 
                  type="number" 
                  step="0.001"
                  placeholder="0.1"
                  class="w-full steem-auth-input"
